...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const product = catalog.products.find((item) => item.sku === productId);
  if (!product) return {};
  const path = `/tampa/nightmare-on-channelside/merch/collection/nightmare-on-channelside/${productId}`;
  const description = `Official Nightmare on Channelside ${product.product_type}. Front and back product imagery show the exact piece.`;
  return {
    title: `${product.display_title} — Nightmare on Channelside`,
    description,
    ...
    openGraph: {
      ...
      description,
      url: path,
      images: product.primary_image_url ? [{ url: product.primary_image_url, alt: product.display_title }] : undefined,
    },
    twitter: {
...
export default async function ProductPage({ params }: Props) {
  const { slug, product: productId } = await params;
  const catalog = await getMerchCatalog();
  const collection = catalog.collections.find((item) => item.slug === "nightmare-on-channelside");
  const product = catalog.products.find((item) => item.sku === productId);
  ...
  const productPath = `/tampa/nightmare-on-channelside/merch/collection/nightmare-on-channelside/${product.sku}`;
  const related = catalog.products
    .filter((item) => item.sku !== product.sku)
    .sort((a,b) => Number(b.product_type === product.product_type) - Number(a.product_type === product.product_type))
    .slice(0,4);
  ...
  const productSchema = {
    ...
    description: `Official Nightmare on Channelside ${product.product_type}. Front and back product imagery show the exact piece.`,
  ...
  return (
    <main className={`${premium.shell} ${styles.shell} ${upgrade.shell} ${final.cinematicShell}`} style={vars}>
      ...
      <div style={{padding:"14px clamp(18px,3vw,42px)",borderBottom:"1px solid rgba(255,255,255,.08)",fontSize:9,fontWeight:900,letterSpacing:".13em",textTransform:"uppercase",color:"rgba(255,255,255,.55)"}}>
        <Link href="/tampa/nightmare-on-channelside/merch/shop" style={{color:"inherit",textDecoration:"none"}}>SHOP</Link> / <span>NIGHTMARE ON CHANNELSIDE</span> / <span style={{color:"#fff"}}>{product.display_title}</span>
      </div>
      ...
      <section style={{display:"grid",gridTemplateColumns:"minmax(0,1.35fr) minmax(340px,.65fr)",gap:24,padding:"26px clamp(18px,3vw,42px) 34px",alignItems:"start",borderBottom:"1px solid rgba(255,255,255,.08)"}} className="noc-product-detail-overhaul">
        <div>
          <div style={{display:"grid",gridTemplateColumns:back?"1fr 1fr":"1fr",gap:12}} className="noc-product-dual-main">
            {front ? <figure style={{margin:0,background:"#090605",border:"1px solid rgba(255,255,255,.10)",borderRadius:14,padding:12,textAlign:"center"}}><img src={front} alt={`${product.display_title} front`} style={{width:"100%",height:"min(62vw,620px)",objectFit:"contain",display:"block",background:"#080503"}}/><figcaption style={{paddingTop:8,fontSize:9,fontWeight:900,letterSpacing:".14em",color:"rgba(255,255,255,.58)"}}>FRONT</figcaption></figure> : null}
            {back ? <figure style={{margin:0,background:"#090605",border:"1px solid rgba(255,255,255,.10)",borderRadius:14,padding:12,textAlign:"center"}}><img src={back} alt={`${product.display_title} back`} style={{width:"100%",height:"min(62vw,620px)",objectFit:"contain",display:"block",background:"#080503"}}/><figcaption style={{paddingTop:8,fontSize:9,fontWeight:900,letterSpacing:".14em",color:"rgba(255,255,255,.58)"}}>BACK</figcaption></figure> : null}
          </div>
          {extras.length ? <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:8,marginTop:10}}>{extras.map((image,index)=><div key={`${image}-${index}`} style={{border:"1px solid rgba(255,255,255,.10)",borderRadius:10,background:"#080503",padding:6}}><img src={image} alt={`${product.display_title} detail ${index+1}`} style={{width:"100%",aspectRatio:"1",objectFit:"contain",display:"block"}}/></div>)}</div> : null}
        </div>

        <aside style={{position:"sticky",top:96,padding:"8px 4px 0"}}>
          <Link className={styles.back} href="/tampa/nightmare-on-channelside/merch/shop">← BACK TO SHOP ALL</Link>
          <span style={{display:"block",marginTop:22,color:"#ffd978",fontSize:9,fontWeight:900,letterSpacing:".2em",textTransform:"uppercase"}}>NIGHTMARE ON CHANNELSIDE · OFFICIAL EVENT MERCH</span>
          <h1 style={{fontFamily:'Didot,"Bodoni MT","Times New Roman",serif',fontSize:"clamp(42px,4.5vw,62px)",lineHeight:.92,letterSpacing:"-.04em",margin:"14px 0 12px",color:"#fff7e7"}}>{product.display_title}</h1>
          ...
          <p style={{color:"rgba(255,255,255,.66)",fontSize:13,lineHeight:1.7,margin:"18px 0 22px"}}>Official Nightmare on Channelside {product.product_type}. Front and back product imagery show the exact piece before purchase.</p>

          {canSell ? <AddToBag variants={product.variants} options={product.options} productTitle={product.display_title} /> : <div className={styles.dropLocked}><strong>GONE DARK</strong><span>This piece is currently unavailable.</span></div>}

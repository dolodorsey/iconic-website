...
export default async function CollectionPage({ params }: Props) {
  ...
  return (
    <main className={`${styles.shell} ${upgrade.shell} ${final.cinematicShell}`} style={vars}>
      ...
      <section className={final.collectionCampaignStrip} aria-label={`${collection.name} campaign world`}><img src={scene} alt={`${collection.name} inside the Nightmare on Channelside campaign world`} /></section>

      <section className={final.collectionProductsWrap} id="products">
        <div className={final.collectionProductsHead}><span>THE DROP</span><h2>WEAR {collection.mood}.</h2></div>
        <div className={final.collectionProductGrid}>
          {products.map((product) => (
            <Link href={`/tampa/nightmare-on-channelside/merch/collection/${collection.slug}/${product.sku}`} className={final.productCardCinematic} key={product.shopify_product_id}>
              <div className={final.productStageBlack}>{product.primary_image_url ? <img src={product.primary_image_url} alt={product.title} className={final.productStageImage} /> : <div className={styles.teeShape}><span>{collection.name}</span><b>{String(product.design_number).padStart(2,"0")}</b></div>}</div>
      ...
      <footer className={final.collectionFooter}><strong>ICONIC</strong><Link href="/tampa/nightmare-on-channelside/merch">← ALL WORLDS</Link><span>NIGHTMARE ON CHANNELSIDE · TAMPA, FL</span></footer>
    </main>
  );
}

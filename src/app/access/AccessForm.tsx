...
export default function AccessForm({intent,event}:{intent:string;event?:string}){
  ...
  if(status==="success") return <div style={{padding:30,border:"1px solid rgba(216,180,100,.45)",borderRadius:22,background:"rgba(216,180,100,.08)"}}><div style={{fontSize:9,fontWeight:900,letterSpacing:".2em",color:"#d8b464",textTransform:"uppercase"}}>Request received</div><h3 style={{fontFamily:"Georgia,serif",fontSize:38,lineHeight:1,margin:"14px 0"}}>You’re in the ICONIC system.</h3><p style={{color:"rgba(255,255,255,.7)",lineHeight:1.7,margin:0}}>Your {title.toLowerCase()} request has been captured and can now be routed to the correct ICONIC lane.</p></div>;
  ...
  return <form onSubmit={submit} style={{display:"grid",gap:16,padding:24,border:"1px solid rgba(255,255,255,.12)",borderRadius:24,background:"rgba(255,255,255,.025)"}}>
    ...
    <div style={{fontSize:10,lineHeight:1.6,color:"rgba(255,255,255,.45)"}}>By submitting, you agree that ICONIC LIVE / The Kollective may contact you about this request. See our <Link href="/privacy" style={{color:"#d8b464"}}>Privacy Policy</Link> and <Link href="/terms" style={{color:"#d8b464"}}>Terms of Use</Link>.</div>
  </form>;
}

...
function template(route){if(/\/merch\/collection\/[^/]+\/[^/]+$/.test(route))return 'merch-product';if(/\/merch\/collection\/[^/]+$/.test(route))return 'merch-collection';if(/^\/summer-walker\/.+/.test(route))return 'summer-city';if(/^\/dj-snake-pardon-my-french\/.+/.test(route))return 'pmf-city';return route}
...
(async()=>{await fs.mkdir(out+'/screenshots',{recursive:true});let ready=false;for(let i=0;i<90;i++){try{const r=await fetch(base+'/api/release');if(r.ok){const body=await r.json();if(!report.expectedProductionSha||body.commit===report.expectedProductionSha){report.release=body;ready=true;break}}}catch{}await sleep(5000)}if(!ready)throw Error('Exact target release did not become ready; refusing to audit an old deployment');
 const all=['/partners',...personnelRoutes];report.routeInventory=all;report.ignoredInternalRoutes=[];report.scope='Isolated personnel visual proof: Partners hub + all nine role application routes. No purchases, lead submissions, or outbound messages.';
 ...
 report.finished=new Date().toISOString();await fs.writeFile(out+'/report.json',JSON.stringify(report,null,2));console.log(JSON.stringify({routes:all.length,rendered:report.pages.length,failures:report.failures},null,2));if(report.failures.length)process.exitCode=1;
})().catch(async e=>{report.failures.push({fatal:e.stack||e.message});await fs.mkdir(out,{recursive:true});await fs.writeFile(out+'/report.json',JSON.stringify(report,null,2));console.error(e);process.exitCode=1});

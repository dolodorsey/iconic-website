...
try{
 ...
 for(const [path,intent] of [['/music','music'],['/creators','creator'],['/partners','sponsorship'],['/media','media']]){
  ...
  await p.goto(BASE+path,{waitUntil:'networkidle'});await p.locator('[name="full_name"]').fill('ICONIC QA');await p.locator('[name="email"]').fill('iconic-ui-qa@example.com');await p.locator('button[type="submit"]').click();await p.getByText('Request received',{exact:true}).waitFor({timeout:10000});assert.equal(payload.intent,intent);assert.equal(await p.getByRole('alert').count(),0,'A successful response must not become a form error');await p.unroute('**/api/iconic-leads');await p.unroute('**/api/event-track');
 }

...
try{
 ...
 for(const [path,expectedRole] of personnelForms){
  ...
  await p.goto(BASE+path,{waitUntil:'networkidle'});const form=p.locator('form:has([name="full_name"])').first();assert.equal(await form.count(),1,`${path} personnel form missing`);await form.locator('[name="full_name"]').fill('ICONIC QA');await form.locator('[name="email"]').fill('iconic-personnel-qa@example.com');await form.locator('[name="phone"]').fill('5555550199');const city=form.locator('[name="city"]');await city.selectOption({index:1});
  ...
  await form.locator('button').last().click();await p.getByText('APPLICATION RECEIVED',{exact:true}).waitFor({timeout:10000});assert.equal(payload.requested_role,expectedRole);assert.equal(await p.locator('[role="alert"]').count(),0,'Personnel success must not become form error');await p.unroute('**/api/noc-partner-apply');
 }
